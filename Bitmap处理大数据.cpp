...
class Bitmap
{
    typedef struct Byte {
        unsigned char bit8;
        static const unsigned char mask[8];//用来取得一个字节每一位的辅助数组
        Byte() {
            bit8 = 0;
        }
        //设置该位，就是存储该数
...
const unsigned char Bitmap::Byte::mask[8] = {0x80,0x40,0x20,0x10,0x8,0x4,0x2,0x1};//用来取得一个字节每一位的辅助数组
